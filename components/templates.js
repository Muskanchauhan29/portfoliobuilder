...
function ModernPortfolioCard({ portfolio }) {
  ...
  return (
    <div className="relative mx-auto rounded-3xl bg-gradient-to-br from-white/80 to-purple-100/80 backdrop-blur-2xl border border-purple-200 shadow-2xl p-10 md:p-16 flex flex-col items-center max-w-3xl">
      <img src={data.personalInfo?.photo || ''} alt={data.personalInfo?.name || ''} className="w-32 h-32 rounded-full border-4 border-purple-400 shadow-xl mb-6 object-cover" />
      <h2 className="text-4xl font-extrabold text-purple-900 mb-1 text-center drop-shadow-lg">{data.personalInfo?.name || ''}</h2>
      <h3 className="text-2xl font-semibold text-pink-600 mb-4 text-center">{data.personalInfo?.role || ''}</h3>
...
export const TEMPLATES = {
  card: {
    ...
    card: (portfolio) => <PortfolioCard portfolio={portfolio} />,
    label: 'Glass Card',
  },
  modern: {
    background: <ModernGradientBackground />,
    card: (portfolio) => <ModernPortfolioCard portfolio={portfolio} />,
    label: 'Modern Gradient',
  },
  grid: {
    background: <PreviewBackground />,
    card: (portfolio) => <GridPortfolioCard portfolio={portfolio} />,
    label: 'Grid Professional',
  },
  elegant: {
    background: <PreviewBackground />,
    card: (portfolio) => <ElegantPortfolioCard portfolio={portfolio} />,
